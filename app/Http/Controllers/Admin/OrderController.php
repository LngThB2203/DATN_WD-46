...
class OrderController extends Controller
{
    ...
    public function updateStatus(Request $request, $id, StockService $stockService)
    {
        ...
        // Xử lý hủy đơn hàng (không dùng transaction bên ngoài vì cancelOrder() đã có transaction riêng)
        if ($newStatus === OrderStatusHelper::CANCELLED) {
            try {
                // cancelOrder() sẽ tự kiểm tra đã trừ kho chưa và chỉ hoàn kho nếu cần
                $stockService->cancelOrder($order);
                return redirect()->route('admin.orders.show', $order->id)->with('success', 'Hủy đơn hàng thành công');
            } catch (\Exception $e) {
        ...
        try {
            ...
            // Cập nhật trạng thái khác (PREPARING → SHIPPING → DELIVERED → COMPLETED)
            // Các trạng thái này không cần thao tác kho vì đã trừ ở PREPARING
