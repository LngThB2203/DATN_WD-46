...
Route::get('/test-send-notification', function () {
    ...
    foreach ($users as $user) {
        $user->notify(
            new App\Notifications\PromotionNotification(
                'Khuyến mãi test 🎉',
                'Giảm 20% cho đơn từ 500.000đ'
            )
        );
    }
...
Route::post('/send-promotion', [NotificationController::class, 'sendPromotion'])
    ->middleware('auth');



Route::post('/exchange-point', [NotificationController::class, 'exchangePoint']);
