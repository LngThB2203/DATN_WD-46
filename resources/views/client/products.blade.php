        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
            ...
            <form method="GET" action="{{ route('client.products.index') }}" class="d-flex flex-wrap gap-2">
                <input type="text" name="q" value="{{ $search ?? '' }}" class="form-control"
                    placeholder="Tìm kiếm sản phẩm..." style="min-width: 220px;">
                ...
                <div class="card h-100">
                    <a href="{{ route('product.show', $product->slug) }}">
                        <img src="{{ $product->primaryImageModel
                                        ...
                                        : asset('assets/client/img/product/product-1.webp') }}" class="card-img-top"
                            style="height: 250px; object-fit: cover;" alt="{{ $product->name }}">
                    </a>
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <div class="mb-2">
                            ...
                            <span class="text-primary fw-bold">{{ number_format($product->sale_price, 0, ',', '.') }}
                                VNĐ</span>
                            <span class="text-muted text-decoration-line-through ms-2">{{ number_format($product->price,
                                0, ',', '.') }} VNĐ</span>
                            @else
                        ...
                        <a href="{{ route('product.show', $product->slug) }}"
                            class="btn btn-sm btn-outline-primary mt-auto w-100">Xem chi tiết</a>
                    </div>
                </div>
            </div>
            @empty
