...
<section class="py-5">
    <div class="container-fluid container-xl">
        ...
        <form method="POST" action="{{ route('checkout.store') }}">
            @csrf
            {{-- Hidden input giỏ hàng --}}
            <input type="hidden" name="selected_items" value="{{ isset($selectedItems) && !empty($selectedItems)
                        ...
                        <div class="card-body">
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('customer_note') is-invalid @enderror"
                                    name="customer_note" rows="3">{{ old('customer_note') }}</textarea>
                                @error('customer_note')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="card mt-4">
                        ...
                        <div class="card-body">
                            ...
                            <div class="form-check">
                                ...
                                <label class="form-check-label" for="payment_online">Thanh toán online
                                    (VNPay/MoMo)</label>
                            </div>
                            ...
                            <div class="alert alert-secondary mt-3" id="bank_instructions" style="display:none">
                                ...
                                <p class="mb-0">Nội dung chuyển khoản: <strong>Thanh toán đơn hàng #{{
                                        now()->format('His') }}</strong></p>
                            </div>

                            <div class="alert alert-info mt-3" id="online_instructions" style="display:none">
                                <h6 class="fw-semibold mb-2">Thanh toán online</h6>
                                <p class="mb-0">Bạn sẽ được chuyển đến cổng thanh toán VNPay/MoMo để hoàn tất thanh
                                    toán.</p>
                            </div>
                        </div>
                    </div>
