                    ...
                    <div class="mb-3 p-3 border rounded bg-light">
                        ...
                        @foreach($order->details as $detail)
                            <div class="d-flex mb-2">
                                @php
                                    $primaryImage = $detail->product->galleries->where('is_primary', true)->first();
                                @endphp
                                <img src="{{ $primaryImage ? asset('storage/' . $primaryImage->image_path) : asset('images/no-image.png') }}"
                                    style="width: 80px; height: 80px; object-fit: cover;"
                                    class="rounded border me-3">
                                {{-- Thông tin sản phẩm --}}
                                <div>
                                    <div class="fw-bold">{{ $detail->product->name }}</div>
                                    <div>Giá: {{ number_format($detail->price) }}₫</div>
